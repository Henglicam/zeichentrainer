...
async function aiReadPicture(blob,alts,status){
  ...
  return {zh,zht:zh!==zhRaw?zhRaw:"",p:await saneP(main.p,zh),m,ml:LANG,note:String(x.note||"").trim(),bad:!!x.bad||!CJK.test(zh),model,pv,box:main.box,boxes:main.boxes,dropped:main.dropped,cut:String(x.cut||"").toLowerCase().replace(/[^a-z,]/g,"")}; /* cut (v314): the edges that cut off a line the model left out */
}
...
function mainLines(lines,p,m,boxes,box){
  const out={lines,p:String(p||""),m:String(m||""),box,boxes:null,dropped:[]};
  ...
  out.dropped=lines.filter((l,i)=>!keep[i]); out.lines=lines.filter((l,i)=>keep[i]); out.boxes=boxes.filter((b,i)=>keep[i]);
...
function snapBox(bmp,box,n,lens){ /* lens: the answer's lines' character counts (v305) */
  ...
  for(const c of [0,1]) comps[c]=comps[c].filter(b=>{ if(b.x1-b.x0<=2*Hb) return true; const cs=comps[1-c].filter(o=>o.clean&&o.y1-o.y0>=0.4*Hb&&o.x1-o.x0<=1.3*(o.y1-o.y0)&&o.x0>=b.x0&&o.x1<=b.x1&&Math.min(o.y1,b.y1)-Math.max(o.y0,b.y0)>=0.5*(o.y1-o.y0)).sort((a,o)=>a.x0-o.x0); let held=0, last=null; for(const o of cs){ if(last&&(o.x0<last.x1||Math.min(o.y1,last.y1)-Math.max(o.y0,last.y0)<0.5*Math.min(o.y1-o.y0,last.y1-last.y0))) continue; held++; last=o; } return held<2; }); /* a plate, not a character (v315, H's parking sign 请您停车入位: the white plate's part inside Qwen's box was 84 % of its width and 78 % of its height — under the background rule's 0.9 — and its area beat the six dark characters, so the plate was the "text", and the light specks on the ground beside it joined it): a blob wider than two text heights that holds a row of two or more character-shaped clean blobs of the other colour — at least 0.4 text heights tall, no wider than 1.3 times their height, within its width and mostly within its rows, side by side without overlap in one band (the plain and the eroded pass see the same gap in 流浪地球's brush title twice — one gap is no row) — is the ground the characters stand on and drops */
  ...
  for(const f of fused[tc]){
    const near=taken.filter(tb=>Math.min(f.y1,tb.y1)-Math.max(f.y0,tb.y0)>=0.5*Math.min(f.y1-f.y0,tb.y1-tb.y0)&&Math.max(f.x0-tb.x1,tb.x0-f.x1)<=0.5*Hb); if(!near.length) continue;
    ...
    const tb=near.reduce((a,c)=>Math.abs((c.x0+c.x1)/2-(f.x0+f.x1)/2)<Math.abs((a.x0+a.x1)/2-(f.x0+f.x1)/2)?c:a), tx0=tb.x0-R.x0, tx1=tb.x1-R.x0;
  ...
  for(const side of [-1,1]) for(let it=0;it<3;it++){
    ...
    const sx0=side<0?Math.max(0,Math.round(edge-1.5*Hl)):edge, sx1=side<0?edge:Math.min(W,Math.round(edge+1.5*Hl)), sy0=Math.max(0,Math.round(tb.y0-0.1*Hl)), sy1=Math.min(Hh,Math.round(tb.y1+0.1*Hl)), sw=sx1-sx0, sh=sy1-sy0;
  ...
  for(const side of [1,-1]){ /* every answer since v326 (H's street sign 北 金汇路 南: Qwen's one-line box sat a tenth too low, the Chinese line lay mostly outside it, the snap kept the pinyin line under it and the card showed N JINHUI LU S with the characters' feet — until v325 only a two-line answer was looked around) */
    ...
    for(let y=0;y<sh;y++){ let px=0, runs=0, mn=sw, mx=-1, prev=0; for(let x=0;x<sw;x++){ const v=(g[(sy0+y)*W+sx0+x]>cut?1:0)===tc?1:0; if(v){ px++; if(!prev) runs++; if(x<mn) mn=x; if(x>mx) mx=x; } prev=v; } rows.push({px,runs,mn,mx}); }
...
async function dropAuto(id,cid){ delete AUTO[id]; delete QSCARD[id]; if(!cid) return; const d=cardOf(cid); if(!d||d.c) return; S.custom=S.custom.filter(x=>x.id!==cid); try{ await idbDel("custom",cid); }catch(e){} bump("byPhoto",-1); dropThumb(cid); setStats(); }
...
async function picOnBad(sg,guesses,status){
  ...
  try{ const pic=await aiReadPicture(sg.picBlob,[...new Set(guesses.filter(Boolean))].slice(0,6),status||(()=>{})); if(sg.region) sg.region.pic={zh:pic.zh,bad:pic.bad,model:pic.model,box:pic.box,boxes:pic.boxes,dropped:pic.dropped}; return pic&&!pic.bad?pic:null; }
...
function renderShots(){
  ...
  box.querySelectorAll("[data-resedit]").forEach(b=> b.onclick=()=>{ const cid=QSCARD[b.dataset.resedit]; if(!cid) return; S.editing=cid; S.editFrom="camera"; S.editOpenFrame=true; S.fullPic=false; render(); window.scrollTo({top:0}); }); /* Edit opens the form with the photo and the frame the card was cut with */
  box.querySelectorAll("[data-resdel]").forEach(b=> b.onclick=async()=>{ const cid=QSCARD[b.dataset.resdel]; if(cid) await delCustom(cid); renderShots(); }); /* at once, with Undo (v268) — Undo brings the result back, the photo stays with Crop meanwhile */

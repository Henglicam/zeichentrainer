...
const APP_V=454; /* must equal the PWA vN label in index.html — the boot check repairs a shell whose files are of different versions */
...
/* a fingerprint of the system prompt (v399): picSystem() is 5 700 characters and changed at v361, v363, v367, v377 and v449, so an
...
async function aiReadPicture(blob,alts,status,rec){
  ...
  const sys=picSystem(); if(rec) rec.prompt=[sys.length,strHash(sys)]; /* v399: picSystem() is 5 700 characters and changed at v361, v363, v367, v377 and v449, so an answer cannot be attributed to a prompt without a fingerprint — the version says which code, this says which words went out */

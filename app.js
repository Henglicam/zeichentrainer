...
function buildQueue(includeAhead){
  ...
  /* the order of the session (v512, H: "Karten mit Charaktern, die häufig vorkommen, werden öfters angezeigt. Schreiben dieser
     ...
     highest first — the mean over the card's characters of freq/(1+writes), freq the number of learnable cards carrying the
     character and writes how often it was fully written (setting charWrites). A card of common, never-written characters
     comes first; every full write of a character halves its pull. Oldest and Newest decide among equal weights; Random
...
const ERRLOG=[], LAST_READ={ring:[]}, AILOG=[]; /* AILOG: the last three AI exchanges, request and raw reply, never the key (v97) */
...
/* The reading's own numbers, beside its sentences (v399, H, 2026-09-10: "Ich werde dann in Zukunft einfach lauter Bilder
   ...
   the phone disagree the harness is wrong, and nothing is tuned against numbers the phone did not send. Device only —
   the daily usage row carries counts and reportErrors() and never this. */
/* One record per reading, keyed by the photo it reads (v399). H's workflow is "lauter Bilder knipsen" and a panel's
   reading takes 15-20 s, so a second photo lands inside the first reading's window as a matter of course. A single
   global would be wiped by the newcomer and the survivor would carry one photo's answer under the other photo's id
   (measured on the first cut of this change: photo B taken while A's picture call was out left A's eleven cards with
   no record at all, and B's record holding A's eleven labels). So the reading writes into its own object and reaches
   it by id; a record that is gone takes its writes with it instead of landing in someone else's. */
const NUMSOF={}, NUMS_LIVE=6; /* the readings in flight and the few just finished — this is the handoff to the ring */
const numsFor=id=>NUMSOF[id]||null;
function numsGC(){ const ks=Object.keys(NUMSOF); if(ks.length<=NUMS_LIVE) return; ks.sort((a,b)=>(NUMSOF[a].at||0)-(NUMSOF[b].at||0)); while(ks.length>NUMS_LIVE) delete NUMSOF[ks.shift()]; }
function numsReset(id,keep){ /* proposeFrame writes the proposal before cropSign starts, as it writes its log line (v285): those fields stay at the head of the reading that follows */
  const old=NUMSOF[id], pre=keep&&old&&old.pre?{photo:old.photo,layer:old.layer,prop:old.prop}:null;
  const N={v:APP_V,shot:id,at:Date.now()};
  if(pre) for(const k of Object.keys(pre)) if(pre[k]!==undefined&&pre[k]!==null) N[k]=pre[k];
  /* v479: the same carry-over for the steps, which live on the record now. The frame's own lines — the proposal, the old crop
     found — are pushed before the reading opens and are marked pre, so the tail of them follows the record that replaces this
     one, exactly as v285's wipe in cropSign used to keep them. Capped, or a photo proposed and read twice stacks them up. */
  if(old&&Array.isArray(old.log)){ const tail=[]; for(let i=old.log.length-1;i>=0&&old.log[i].pre&&tail.length<LOG_PRE_KEEP;i--) tail.unshift(old.log[i]); if(tail.length) N.log=tail; }
  /* and the photo keeps its one place in the ring: a re-read replaces its record where it stood rather than taking a second
     slot, so three ring entries are three photos and never one photo three times */
  { const r=LAST_READ.ring, i=old?r.indexOf(old):-1; if(i>=0){ r[i]=N; N.ringed=true; } }
...
/* Every line of the reading's later stages goes through here (v374, H's washing machine: the diagnostics arrived after a reload
   and ended at "Asking the AI about the picture …" — the split's own lines were pushed straight into a global list, so the
   reasons a panel did not split were never in a diagnostics text). It pushes, trims and saves like readingStatus.
   THE LINE BELONGS TO A PHOTO (v479, H's dump of 2026-09-13: "the AI could not check the photo and the reading is weak — no
   card" stood at step 12 of 16, before two further recognizing steps of a different photo). READLOG was one global 40-line
   list and cropSign wiped it at its start, so a reading that began while an earlier one was still finishing — the album batch
   of v411, where the next photo starts as soon as the last has its cards, while that one's AI check and finishPending are
   still running — erased its predecessor's steps and then collected its predecessor's tail among its own. v406 named it and
   left it. The steps live on the reading's own numbers record now, which is already keyed by photo and already kept three
   deep (v399/v405), so a step can no longer be filed under the wrong photo and the two earlier readings keep their steps
   instead of only their numbers. */
function logRec(id){ if(!id) return null;
  let N=NUMSOF[id]||LAST_READ.ring.find(x=>x&&x.shot===id)||null;
  if(!N){ N=NUMSOF[id]={v:APP_V,shot:id,at:Date.now(),pre:true}; numsGC(); } /* findFrame logs before any record exists */
  if(!Array.isArray(N.log)) N.log=[];
  ringAdd(N);
  return N; }
/* the ring is in the order the readings started, which is the order the blocks are printed in. A record joins it once, at
...
const numBox=b=>b?[n1(b.x0),n1(b.y0),n1(b.x1),n1(b.y1)]:null; /* a box in a copy's own pixels */
const numFrac=b=>Array.isArray(b)&&b.length===4?b.map(n4):null; /* a box as the model gave it, in fractions of the picture it saw */
const numAdd=(id,k,v)=>{ const N=numsFor(id); if(N) (N[k]||(N[k]=[])).push(v); }; /* v399: the id, so a refusal logged by a reading that is over cannot land in the one running now */
const numSet=(id,k,v)=>{ const N=numsFor(id); if(N) N[k]=v; };
/* the record names the cards it made (v399): H sends a screenshot of a card he thinks is wrong, and the record has to be
   findable from it — and if the photo itself is needed, as it was at v386, the shot id says which one to ask for */
const numCards=(id,ids)=>numSet(id,"cards",ids.slice(0,40));
const numFrames=fs=>{ const a=fs.filter(Boolean)[0]; return a?{lw:n1(a.lw),lh:n1(a.lh),f:fs.map(r=>r?[n1(r.x),n1(r.y),n1(r.w),n1(r.h),n1(r.a||0)]:null)}:null; }; /* many frames of one layer: the layer once, the rectangles after it */
/* the picture answer as every later decision reads it (v399, measured on H's washing machine: the whole object is 2 931
   bytes for 19 labels, this is 1 390 — 73 a label —, and it is what templateBoxes, roundGrid, picBox/picScale, mainLines
   and snapBox all consume. The raw reply stays in AILOG, where v395 gave it 6 000 characters; this is the same answer
   after the app has read it, so the two can be compared and a misreading of the model's numbers shows up as the
...
function numsTrim(o,cap){ const max=cap||NUMS_MAX; if(!o) return o;
  const o2={...o}; delete o2.log; delete o2.passes; delete o2.ringed; /* v479: the steps and the passes live on the record now and are printed as their own lines — inside this JSON they would eat the numbers' own budget; ringed is bookkeeping */
  if(JSON.stringify(o2).length<=max) return o2;
  const c={...o2}; for(const k of ["qk","lrects","best","found"]){ if(c[k]!==undefined){ c[k]="dropped, over "+max+" characters"; if(JSON.stringify(c).length<=max) break; } } return c; }
/* a fingerprint of the system prompt (v399): picSystem() is 7 200 characters and changed at v361, v363, v367, v377, v449 and v455, so an
...
function diagText(){
  ...
  const out=[`Zeichentrainer diagnostics — ${new Date().toLocaleString("en-GB")}`,
    ...
    /* what the pipeline actually ran on (v399): poolSize() gives 1 on a phone of 3 GB or less and 3 or 4 otherwise, and the
       ...
       be reproduced without it. The storage numbers say whether the diagnostics are still being written down (see STORAGE). */
  ...
  const stepLines=(N,ind,head,tail)=>{ const a=(N&&N.log)||[], o=[], drop=head&&a.length>head+tail?a.length-head-tail:0;
    a.forEach((x,i)=>{ if(drop&&i===head) o.push(`${ind}\u2026 ${drop} step${drop===1?"":"s"} left out`);
      if(!drop||i<head||i>=head+drop) o.push(`${ind}${ago(x.t)}  ${x.text}`); });
    return o; };
  const now=lastRead();
  out.push(`Last reading (${now&&now.log?now.log.length:0} steps)${now&&now.shot?" \u00b7 "+now.shot:""}:`);
  if(now){ out.push(...stepLines(now,"  ",0,0));
    if(now.passes) out.push("  passes: "+JSON.stringify(now.passes));
    out.push("  numbers: "+JSON.stringify(numsTrim(now))); } /* the frame chain's own values, unrounded (v399) — the prose above is for reading, this is what a harness is fed */
...
const TO_TEST=[
  ...
  ["app","v518","Learn: swipe from the row / the fold"],
  ...
  ["app","v512","Progress: Written today counts"],
...
async function boot(){
  try{
    ...
    resumeShots(); /* v509: a photo the app was still working on when it was closed is back in the queue before the first render, so the Camera tab shows it waiting; a raw one waits for normalizeRaw below */
...
async function aiReadPicture(blob,alts,status,rec){
  ...
  const txt=raw.trim().replace(/^```(?:json)?\s*|\s*```$/g,""); let x; try{ x=JSON.parse(txt); if(Array.isArray(x)) x=x[0]; }catch(e){ x=mendJSON(txt); if(!x) throw new Error("could not read the model's answer");
    logRead(rec&&rec.shot,`the AI's answer stopped in the middle — kept what came (${Object.keys(x).join(", ")})`); }
  ...
  { const keep=lines0.map(l=>CJK.test(l));
    if(keep.some(Boolean)&&!keep.every(Boolean)){
      ...
      logRead(rec&&rec.shot,`a line of Latin letters left out of the AI's answer: ${lines0.filter((l,i)=>!keep[i]).join(" | ")}`);
...
function aiQueue(){ return deck().filter(d=>d.c&&!isPage(d)&&!aiRefused(d)&&(d.flag||(d.mt&&(d.mt.pending||d.mt.suspect)))); } /* a card still waiting for its reading has no text to check (v237) */
...
const setMl=(card,ml)=>{ if(ml&&ml!=="en") card.ml=ml; else delete card.ml; if(card.m) card.ms={...(card.ms||{}),[ml||"en"]:card.m}; return card; };
/* the fuller description (v529, H: "Meaning unten kann dann noch ausführlichere Beschreibung enthalten", then "Go for the ausführlichere
   Beschreibung"): two or three sentences the AI writes about the card in the app's language — what the text says in context, its
   key characters, where one meets it — stored per language as ds = {de:"…", en:"…"} (the ms shape of v265), so a language switch
...
/* what an automatic check would be about: the card's text, pinyin and meaning. A dismissal is recorded against it (v535),
   so the same answer cannot be asked for and stored again on the next start - and an edit, an accepted suggestion or a
   new reading changes the fingerprint by itself, which makes the card checkable again with no second field to clear. */
const aiFp=d=>(d.c||"")+"\u0000"+(d.p||"")+"\u0000"+(d.m||"");
const aiRefused=d=>!!d.aiNo&&d.aiNo===aiFp(d);
function dismissed(d){
  ...
  upd.aiNo=aiFp(d); /* v535 (H: "The same AI suggestion shouldn't pop up again and again"): the flag and mt.pending stay, and both keep a card in the automatic run for ever - so the run has to know the answer was turned down */
  return upd;
}
...
/* v512: the day's entry is {r,w} — reviews and points written on the pad; a plain number from before reads as {r:n,w:0} */
const dayOf=x=>typeof x==="number"?{r:x,w:0}:{r:(x&&x.r)||0,w:(x&&x.w)||0};
function dailyBump(day,key,n){ const d=S.settings.daily||{}; const e=dayOf(d[day]); e[key||"r"]+=n||1; d[day]=e; const keys=Object.keys(d).sort(); while(keys.length>60) delete d[keys.shift()]; S.settings.daily=d; clearTimeout(_dailyTimer); _dailyTimer=setTimeout(()=>{ setSetting("daily",d).catch(()=>{}); },500); }
...
async function applyStage(st,list){ const rows=[], before={}; for(const x of list){ const d=cardOf(x.id), e=d&&st.m[d.id]; if(!e||e.from!==d.m||e.skip||!e.m) continue; before[d.id]={m:d.m,ml:d.ml,ms:d.ms,ds:d.ds}; const u=setMl({...d,m:e.m},st.lang); if(e.desc) setDesc(u,e.desc,st.lang); rows.push(u); } /* the description in the new language rides with the meaning (v529) */
  if(rows.length){ try{ await idbPutMany("custom",rows); }catch(e){ logErr("translate","apply: "+(e&&e.message||e)); return 0; } for(const r of rows){ const i=S.custom.findIndex(x=>x.id===r.id); if(i>=0) S.custom[i]=r; } await saveLastRun("meanings",["m","ml","ms","ds"],before,rows.length); } return rows.length; }
  ...
  try{
    for(let i=0;i<todo.length;i+=TRANSLATE_BATCH){
      ...
      for(let k=0;k<batch.length;k++){ const d=cardOf(batch[k].id), a=ans[k]; if(!d) continue;
        stage.m[d.id]=a&&a.m&&!a.bad?{m:a.m,desc:a.desc||"",from:d.m}:{skip:true,from:d.m}; TRANSLATE.done++; } /* staged, not written: the cards change together at the end */
...
function reportData(){
  ...
  return {install:installId(), version:APP_V, device:dev, installed:isInstalled(), first:u.first?new Date(u.first).toISOString().slice(0,10):null,
    ...
    regionTaps:n("regionTaps"), regionGrades:n("regionGrades"), regionCards:n("regionCards"), written:n("written"), writtenMonth:mn("written"), /* the points written on the pad (v512) */ /* the photo's sheet opened, a grade given in it on a marked photo (v448), and a multicard's text turned into a flashcard (v478) */
    errors:reportErrors()};
}
...
function progressData(){
  ...
  for(let i=29;i>=0;i--){ const k=dayKey(t0-i*DAY); dots.push({k,n:dayOf(daily[k]).r||(daysSet.has(k)?1:0)}); }
  return {streak:st.streak,learned:st.total,week:st.week,dueToday,dueTomorrow,dueWeek,nw,learning,known,dots,written:dayOf(daily[dayKey(t0)]).w}; /* written: the points of today's writes on the pad (v512) */
}
...
function frontHTML(d,o){
  ...
  if(d.kind==="sign"){
    ...
    return `<div class="signfront">${frontPic(d,o)}
...
/* the description under the meaning (v529): the card's own in the app's language when it has one; else, where the caller allows it
   (explain: the Learn block and the detail — never the camera's finished card, whose check is still on its way), the Explain button,
   which asks the AI once for this card alone. EXPLAIN holds the button's state by card id (busy, or a failure's sentence) so a re-render
   draws it right; the answer is put into the DOM in place (refreshDesc), never through render(), which would rebuild the pad under a finger. */
const EXPLAIN={};
function descHTML(d,o){ const s=descOf(d); if(s) return `<p class="desc" data-desc="${esc(d.id)}">${esc(s)}</p>`;
  if(!(o&&o.explain)||!d.c||!aiOn()) return "";
  const st=EXPLAIN[d.id]; if(st==="busy") return `<div class="desc want" data-desc="${esc(d.id)}">${busyHTML(t("Explaining …"))}</div>`;
  return `<div class="desc want" data-desc="${esc(d.id)}"><button class="del" data-explain="${esc(d.id)}">${t("Explain")}</button>${st?`<div class="hint">${esc(st)}</div>`:""}</div>`; }
  ...
  card.querySelectorAll(".chrow .ch").forEach(b=>{ const i=+b.dataset.i, x=tg[i]; if(!x) return;
    b.onclick=e=>{ e.stopPropagation();
      ...
      /* a tap puts this character into the pad; its word lights and the line under the pad reads it (v517), and since v518
         ...
         under the pad don't follow. And they should Always be visible"), so there is no second tap that puts it out */
    ...
    if(lockOn()&&x.w&&!S.single) longPress(b,()=>S.lockChar&&x.ch===S.lockChar?unlockChar():lockChar(x.ch,c)); }); /* v531: only while the lock is on */ /* v518: pressing and holding the locked character releases it (H: "I don't get out of the character locked mode anymore" — a second hold re-locked it); since v526 the hold is the only release */
...
/* v527: the block sits under the pad, so opening it scrolls the card until the block stands above the tab bar (or, when it is
   ...
   block at the foot takes the same scroll (v531) */
function revealBlock(sel){ const a=$(sel), nav=$("#tabs"), hd=document.querySelector("header"); if(!a) return; const r=a.getBoundingClientRect(), navTop=nav?nav.getBoundingClientRect().top:innerHeight, top=hd?hd.getBoundingClientRect().bottom:0;
...
function padTargets(d){
  ...
  for(const w of words){ const chars=[...w]; let at=-1; for(let k=pos;k+chars.length<=text.length;k++){ if(text.slice(k,k+chars.length).join("")===w){ at=k; break; } }
    if(at<0) break; /* a word the text does not carry from here on (a gloss out of step): the rest goes character by character */
    for(let p=pos;p<at;p++) pushChar(p,wi++,text[p],p); /* characters no word claims are words of one */
    if(NUM_PART.test(w)&&!CJK.test(w)){ out.push({ch:w,glyph:w,pos:at,wi,word:w,wstart:at,w:false}); } else for(let p=at;p<at+chars.length;p++) pushChar(p,wi,w,at);
    wi++; pos=at+chars.length; }
...
/* the lit word (v517, H: "Wenn ich auf ein Wort tippe wird zunächst das ganze Wort gehighlighted und Pinyin und Bedeutung
   für dieses eine Wort dargestellt") is the word of the character the pad is on — a tap puts a character into the pad, so
   the tap lights its word, and the pad moving on lights the next (v518); S.wordCh of v517 is gone with the second tap */
...
/* TRACE_OK: the mean distance, in pad sides, between the drawn stroke's eight points and the template stroke's — the spec's
   ...
   the pad, which puts the glyph itself at about 70 % — room for a finger that overshoots. */
let STROKE_OF=new Map(); /* ch → its medians as the file has them (1024 box, y down), filled by loadStrokes */
const padPt=p=>[(p[0]/1024-0.5)*PAD_FIT+0.5,(p[1]/1024-0.5)*PAD_FIT+0.5]; /* a median point in pad units (0..1) */
function resamplePts(pts,n){ if(pts.length===1) return Array(n).fill(pts[0]); const seg=[0]; for(let i=1;i<pts.length;i++) seg.push(seg[i-1]+Math.hypot(pts[i][0]-pts[i-1][0],pts[i][1]-pts[i-1][1])); const L=seg[seg.length-1]||1e-6, out=[]; for(let k=0;k<n;k++){ const t=L*k/(n-1); let i=1; while(i<seg.length-1&&seg[i]<t) i++; const a=pts[i-1], b=pts[i], f=seg[i]===seg[i-1]?0:(t-seg[i-1])/(seg[i]-seg[i-1]); out.push([a[0]+(b[0]-a[0])*f,a[1]+(b[1]-a[1])*f]); } return out; }
/* the forward distance alone (§ 6): a stroke drawn backwards is refused, which is what a tracing exercise is for — strokeDist
...
function mountPad(card,d,c,tg,st,cur){
  ...
  const charDone=async helped=>{
    ...
    /* v535 (H: "Bitte immer erst ein Wort ausschreiben und dann die anderen Characters"): the pad finishes the word it
       is in before it looks anywhere else. Until v534 it took the first unwritten character of the CARD, so tapping 供 of
       鸡蛋供应 and writing it sent the pad back to 鸡 with 应 left standing — writing in the card's order hid it, a tap
...
function swipeHint(d){ return showHints()?`<div class="hint">${t("Trace the lit stroke; the pad moves on by itself.")}${curList().length>1?" "+t("Swipe left or right to pick another card."):""}${lockOn()?" "+t("Press and hold a character to walk through every card that has it; press and hold it again to come back."):""}</div>`:""; } /* v512: the reveal hint went with the reveal */
...
/* grade() went with the vote (v512): the pad's cardDone writes the review through recordGrade — a full write is "good", a card finished with a Skip is "again" (Q10), a swipe writes nothing */
    ...
    if(aiLive()){
      ...
      try{
        ...
        if(r&&!r.bad&&(r.p||r.m)){
          ...
          saveDraft(); S.draft.ai={c:word,p:r.p||"",m:r.m||"",ml:r.ml,desc:r.desc||""}; return;
        }
...
function renderEdit(main,c){
  ...
  $("#e-save").onclick=async()=>{
    ...
    if(aiMl&&!meanTouched) setMl(upd,aiMl); else if(meanTouched||mean!==d.m) setMl(upd,LANG);
    if(aiDesc) setDesc(upd,aiDesc.s,aiDesc.ml); /* v529 */ /* a meaning typed here is in the app's language, one the AI filled in carries its own; an untouched meaning keeps its language (v256) */
...
async function addManual(){
  ...
  const card={id:cardId(word),c:word,p:pin,m:mean,t:"Custom",at:Date.now(),v:APP_V};
  const ai=S.draft&&S.draft.ai; if(ai&&ai.c===word&&(!ai.p||ai.p===pin)&&(!ai.m||ai.m===mean)){ card.mt={src:"llm",verified:true,pending:false}; if(ai.m) setMl(card,ai.ml); if(ai.desc) setDesc(card,ai.desc,ai.ml); } /* filled in by the AI and left as it was (v159); the meaning's language with it (v256) */
  ...
  if(S.pendingShot){ card.shot=S.pendingShot; todoDone(S.pendingShot); S.pendingShot=null; } /* v509: the photo has its card */
...
/* THE LOCK IS OFF, AND KEPT (v531, H: "Deaktiviere die lock Funktion erstmal wieder, aber verwerfe sie nicht. Ich werde sie
   später evtl. reaktivieren."): the long press on a character that locks it and walks every card carrying it (v513, v526,
   v528) is behind this one switch. Off, the hold does nothing, the hint and the guide do not name it, and a resume note
   still carrying a lock is ignored — S.lockChar is then never set, so every lock surface (the chip, the outline, the line's
   padlock, the walk, the spotlight) stays inert by construction. Nothing is deleted: lockChar, unlockChar, walkOf, spotChar,
   the CSS and the keys stay, and true brings all of it back; so does the setting lockOn, which is the harness's way in for
   the lock suites (it has to hold across a reload, where a page variable would not) and nothing in the app writes. */
const LOCK_ON=false, lockOn=()=>LOCK_ON||S.settings.lockOn===true;
/* one place for the phone's buzz (v518, H on the lock's: "Vibration there doesn't work"): the 12 ms of v354/v513 is under what
   most motors render at all, and cannot be measured here — 40 ms for a hold, two pulses for a release, 20 for a finished
   character; a phone with no vibrator, or one that refuses, is silent and nothing else changes */
function buzz(p){ try{ return !!(navigator.vibrate&&navigator.vibrate(p)); }catch(e){ return false; } }
...
async function findFrame(fullBlob,cropBlob,id){ /* id (v479): the line below is logged before the reading opens, and it is this photo's */
  let F=null,C=null;
  try{
    ...
    logRead(id,`the old crop ${best<=22?"found":"not found"} in the photo (difference ${best.toFixed(1)})`,true);
...
/* a batch of photos becomes a batch of cards, one after the other (v411, H: "Geht background processing in der Android app?" —
   no, and it never will: the web has no API for sustained background compute, Chrome throttles a hidden page's timers to about
   one task a second and then a minute, then freezes it, then discards it, and MIUI is harder still; a TWA changes none of that,
   since it is the same Chrome renderer. What the app CAN do is work through a batch while it is open, which until v410 it did
   not: importPhotos opened the FIRST photo of an album and left the other nine in the inbox with a Crop button, untouched until
   each was tapped. Now they queue.) The queue holds only photos from an explicit import — never the inbox at large, which on
   H's phone is 237 photos and would be 237 AI calls. One at a time, since the reader's pool is shared; a frame standing or any
...
function sureLines(lines){ return lines.length>0&&lines.every(l=>{ const cf=l.cf||[]; return cf.length>0&&cf.reduce((a,c)=>a+c,0)/cf.length>=PLACE_CF; }); } /* every line of the reading read at PLACE_CF on average (v441): the gate for the card shown while the AI still checks it. Measured over the eleven strong readings of the corpus: the seven clean ones have their weakest line at 96.7–99, the four garbage or mixed ones — the Panasonic panel 79, Taobao 88.5, the dad poster 89, the menu 92.2 — under 95; effScore cannot tell them apart (keep 185–250 against block 183–301), and a reading-wide mean separates them by 0.03 points. A per-line number, since one garbage line beside real ones is the shape of a mixed reading. */
...
const AUTO_LW=440; /* the layer width H's phone reports for the Camera tab (v411) */
async function proposeFrame(id){
  ...
  if(!layer&&!(S.autoCard&&!RECROP[id])) return; /* a frame the user will see needs its layer; a card made by itself does not (v411) */
  if(img&&!img.complete) await new Promise(r=>{ img.onload=r; img.onerror=r; });
  if(!same()) return;
  /* the proposal is a rectangle on the photo, and the layer is only how it is shown — every step after this divides by lw/lh.
     Off the Camera tab there is no layer to measure, so a nominal one stands in (AUTO_LW, the width H's own records show for
     the Camera tab); the only absolutes downstream are the 8-pixel floor in cropBlob and CROP_MIN, and they then behave as they
     do at 440. A photo that IS on screen keeps its real layer, so nothing about the existing paths moves. */
  const r=layer?layer.getBoundingClientRect():{width:AUTO_LW,height:AUTO_LW*(PH||3)/(PW||4)};
  ...
  const pc=v=>Math.round(v*100); logRead(id,`frame proposed by the app${hidden?" (not shown)":""}: ${full?"the whole photo":`${pc(f.x/r.width)}–${pc((f.x+f.w)/r.width)} % across, ${pc(f.y/r.height)}–${pc((f.y+f.h)/r.height)} % down${shaped?" (16:9)":" (the text's own box, 16:9 does not fit)"}`}${whole0&&reg0?` — ${shared?"a shared screenshot":"a screenshot from the album"} is read whole, its text rows ${pc(reg0.y)}–${pc(reg0.y1)} % set aside`:reg?`, text rows ${pc(reg.y)}–${pc(reg.y1)} %`:", no text rows found"}`,true);
  if(hidden){
    cropSign(id);
    ...
    renderShots();
    setTimeout(()=>{ if(!(CROP&&CROP.id===id&&CROP.hidden)) return; delete CROP.hidden; logRead(id,`frame shown as proposed — the reader took longer than ${FRAME_WAIT/1000} s`); renderShots(); },FRAME_WAIT); /* the reader is slower: the proposal becomes the frame (v289); the reader may still move it once onto the text it finds, while no finger has touched it (v310) */
...
async function cropBlob(id,rectArg){
  ...
  return blob?{blob,X,Y,pw:PW,ph:PH}:null; /* pw/ph (v399): the photo's own pixel size, for the reading's record — every caller already has the bitmap decoded here */
}
...
/* The geometry of a placement, on its own (v399). Everything frameOnText does to turn a rectangle of the straightened copy
   into a frame on the layer — the refine turn of v303, the turned frame of v297, the trim of v333/v334, the upright
   fallback of v311, the unrotated box, the grow of v313 — is arithmetic on numbers the reading's record now carries, so it
   is a function of them and nothing else. That is what makes the record checkable rather than merely stored: a fresh
   harness given only NUMS can call this and must land on the same frame to the pixel. frameOnText keeps every guard, every
   side effect and every log line; only the arithmetic moved. */
function frameGeom(base,rect,angle,W,Hh,grow,sure,refine){
  ...
  return {nr,upright,trimmed,sticks};
}
const cjkOnly=s=>[...String(s||"")].filter(c=>CJK.test(c)).join(""); /* v486: the characters alone — punctuation, spaces and line breaks must not decide whether two readings are the same text */
async function frameOnText(id,orig,base,rect,angle,by,grow,sure){ /* sure (v333): the reading confirms the straightening — a turned frame that would leave the photo is trimmed to it instead of placed upright */ /* grow (v313): room beyond the picture's edges, in the copy's pixels, when the AI's box touches them */ /* rect: the text with its room, in the straightened frame's coordinates; orig = the crop the reading started from, base = the frame it was cut with (v297: a frame the quick look placed meanwhile is not the picture's frame); by = "AI" when the picture answer places it (v293: allowed while the frame is still the app's proposal, shown or not) */
  ...
  if(!base||!rect||RECROP[id]){ numAdd(id,"place",{by:by||"reader",why:!base?"no base frame":!rect?"no rectangle":"the Edit form owns the frame"}); return null; }
  if(pend){ if(!READ_APP[id]||(base.a&&!refine)){ numAdd(id,"place",{by:by||"reader",why:!READ_APP[id]?"the frame is the hand's":"the base is turned"}); return null; } } /* only a frame the app drew may be moved for the waiting card; the hand's frame is the card's */
  else if(!cr||!(CROP.hidden||(CROP.proposed&&(by||!CROP.followed)))||(base.a&&!refine)){ numAdd(id,"place",{by:by||"reader",why:!cr?"no frame stands":base.a&&!refine?"the base is turned":"the frame is no longer the app's"}); return null; } /* the AI may move a frame the reader placed from a weak reading, never one the hand touched (the hand's first gesture drops `proposed`); a turned base is the hand's — unless it is the placed frame itself. The proposal shown by the 2 s fallback is still the app's, so the reader may move it once (v310, H's 风流一代 poster: the reading proper took longer than 2 s, the whole-photo proposal was shown, then the close look read the title at 97 % and the frame stayed the whole photo — "Warum wurde hier nichts cropped?"; H: "Do A") */
  let W=0,Hh=0; try{ const bmp=await createImageBitmap(orig); W=bmp.width; Hh=bmp.height; bmp.close(); }catch(e){ return null; }
  if(!W||!Hh||(!pend&&(!CROP||CROP.rect!==cr))){ numAdd(id,"place",{by:by||"reader",why:!W?"the copy could not be decoded":"the frame moved meanwhile"}); return null; }
  const g=frameGeom(base,rect,angle,W,Hh,grow,sure,refine); /* v399: the arithmetic is a function of the numbers alone, and the reading's record carries them */
  if(!g){ numAdd(id,"place",{by:by||"reader",why:"too small"}); return null; }
  const {nr,upright,trimmed,sticks}=g;
  if(!(cr.a||0)===!nr.a&&Math.abs(nr.x-cr.x)<0.01*nr.lw&&Math.abs(nr.y-cr.y)<0.01*nr.lh&&Math.abs(nr.w-cr.w)<0.01*nr.lw&&Math.abs(nr.h-cr.h)<0.01*nr.lh){ numAdd(id,"place",{by:by||"reader",rect:numBox(rect),copy:[W,Hh],base:numRect(base),ang:n4(angle),out:numRect(nr),why:"nothing to move"}); return null; } /* the text fills the frame: nothing to move */ /* v346 tripped over this silence: the log carried no placement line at all and the card's picture was the whole proposal */
  ...
  numAdd(id,"place",{by:by||"reader",rect:numBox(rect),copy:[W,Hh],base:numRect(base),ang:n4(angle),grow:grow?[n1(grow.top),n1(grow.bottom),n1(grow.left),n1(grow.right)]:null,keep:sure&&sure.box?numBox(sure.box):sure?1:0,refine,out:numRect(nr),upright,trimmed,sticks,took:true}); /* v399: every value frameGeom was given, and what it gave back */
  const pc=v=>Math.round(v*100); logRead(id,`frame ${refine?"centred":"placed"} on the text${by?" by the "+by:""}: ${pc(nr.x/nr.lw)}–${pc((nr.x+nr.w)/nr.lw)} % across, ${pc(nr.y/nr.lh)}–${pc((nr.y+nr.h)/nr.lh)} % down${nr.a?`, turned by ${nr.a}°`:""}${trimmed?(sticks?" — trimmed to the text, the rest reaches past the photo's edge":" — trimmed to the photo"):""}${upright?` — upright, the frame turned by ${angle.toFixed(1)}° would leave the photo`:""}`);
  return cut.blob;
}
...
async function cropSign(id,opts){
  ...
  const N=numsReset(id,true); status("cutting out the frame …"); /* v479: the steps are this photo's own, and numsReset carries the frame's lines (proposed by the app, the old crop found) into the new record — the v285 rule, without the global wipe that erased another photo's reading */ /* v399: the numbers of this reading, keeping the proposal proposeFrame put down before it */
  ...
  try{
    ...
    if(stale()) return;
    N.base=numRect(base); N.app=!!READ_APP[id]; N.pend=!!PENDING[id]; N.recrop=!!RECROP[id]; if(r&&r.pw) N.photo=[r.pw,r.ph]; /* v399 */
    ...
    if((CROP&&CROP.id===id&&CROP.hidden)||(PENDING[id]&&!RECROP[id]&&READ_APP[id]&&!PLACED[id])){ /* a quick look for the frame alone (v290; also for a card made by itself, whose frame is the app's until the reader or the AI places it — v325, H: "you don't need to translate first, you just need to identify text first"): one pass on a copy of at most FIRST_MAX px — 0.3 s on H's poster where the whole frame at 1 600 px takes 1.1 s — whose confident boxes give the lines and the image their ends; the frame goes there before the reading proper starts. It is not one of the reading's passes: as the first pass it lost 爸爸 on that poster, so the reading stays as it was */
      ...
      let rect=null, rtxt=""; try{ const read=scaleBoxes(await readPass(w,src,status),k); if(stale()) return; const lines=tallLines(read,Hink), fine=read.filter(l=>!lines.includes(l)); /* fine print beside taller ink places nothing (v320) */
        ...
        logRead(id,`quick look: ${read.length?read.map(l=>l.t).join(" | ")+` at ${Math.round(meanCf(read))} %`:"nothing"}${fine.length?` — fine print beside taller ink, left out: ${fine.map(l=>l.t).join(" | ")}`:""}${lines.length&&!ok?" — not text, no frame from it":read.length&&!lines.length?" — nothing left to frame":""}`); /* Diagnostics (v296) */
        if(ok){ rect=rectOfLines(bmp,lines); rtxt=cjkOnly(lines.map(l=>l.t).join("")); } /* garbage places no frame (v296) */
        ...
        if((!ok||several)&&!EARLY[id]&&Math.abs(dk.angle||0)<SKEW_TRUST&&pictureUp()){
          ...
          EARLY[id].p.then(e=>{ if(stale()||ok||r.done||r.stop||r.passesDone||!e||!e.pic||e.pic.bad||Math.abs(dk.angle||0)>=SKEW_STOP) return; r.stop=Date.now(); /* v457: never on the `several` trigger — there the quick look LIKED the reading, so cutting the passes short would lower effScore, turn a good reading weak and hand the card to the model. v442's stop is for a reading that was going to be garbage anyway. */ logRead(id,"the AI has answered while the reader is still at work — the reading stops at its next step"); }); }
        } finally{ bmp.close(); }
      if(rect){ await placeRect(rect,null,rtxt); if(stale()) return;
        if(CROP&&CROP.id===id&&CROP.hidden){ delete CROP.hidden; logRead(id,"frame shown as proposed — the text fills it"); renderShots(); } } } /* nothing to move: the proposal is the frame, from now */
    status("reading the text …");
    ...
    const trustAngle=sureAngle||Math.abs(dk.angle||0)<SKEW_TRUST; N.sure=sureAngle; N.trust=trustAngle; /* v399 */
      /* the placement the picture answer earns — the snap, the frame, the drawing test, the reader's per-label search and
         SPLIT[id] — is a closure since v406, so the garbage-verdict path reaches it too (H's washing machine at v405: three
         photos, 14 labels answered, 6 cards). Until v406 all of this lived inside the weak path's terminal branch, while
         picOnBad (v302) sits in signAskAI and only swapped the text, so a panel whose garbage reads *confidently* —
         effScore 192.7 and 198.6 against WEAK_READ 180, because the reader does get 混合, 快速, 大件, 羊毛 right — made one
         card whatever the model listed. The body is unchanged; only its wrapper is new. atRun: done(r) bumps READ_RUN itself
         (v290), so the second entry cannot use cropSign's own stale() — it honours the run it was called at, and a newer
         reading of this photo still aborts it. */
      const placeFromPicture=async(pic,picSeen,atRun)=>{
        const stale=()=>READ_RUN[id]!==atRun; /* shadows cropSign's own, which is already one run behind on the second entry */
      ...
      if(pic.box&&picSeen&&(PENDING[id]&&!RECROP[id]?READ_APP[id]:CROP&&CROP.id===id&&CROP.proposed)){ /* the reader could not read this font (v293 — H's 邪不压正 poster: the ink rows and the reader's garbage boxes put the frame around the whole photo): the AI's box places the frame, once, as fractions of the straightened picture it saw — the proposal's crop, or the cut of the frame the quick look had placed from that same garbage; on the placed frame's own cut (v301) the box centres the frame on the characters inside it (v303, H's 绿皮书: "should be more centered") */
        ...
        let W=0,Hh=0,box=null,rect=null,grow=null,altWon=false,labelRects=null,labelWhole=false,splitWhole=false; try{ const b=await createImageBitmap(seen); W=b.width; Hh=b.height; const [bx0,by0,bx1,by1]=pic.box, n=Math.max(1,zh.length);
          ...
          if(pic.boxAlt){ /* the box overshoots the picture by a little (v340, H's ARRI poster 突破光影边界: Qwen's box [120,330,860,450] for an 800×600 picture — pixels, the title running to the right edge —, read on the 0–1000 grid as 12–86 % across, 33–45 % down: the blank blue above the title, so the card showed ARRI and cut the title; v328's far 邪不压正 answered the same way and meant the grid): the numbers read as pixels, clamped to the picture, name another place, and the snap decides — the pixel reading wins when its box holds a line of characters and the grid's holds none */
            ...
            N.snapAlt=s2?{box:numBox(s2),count:s2.count}:null; /* v399 */
            ...
            if(cp>=3&&cg<3){ logRead(id,`the AI's box ${pc(bx0)}–${pc(bx1)} % across, ${pc(by0)}–${pc(by1)} % down passes the picture's edge on the 0–1000 grid and holds no characters there — read as pixels it holds ${cp} blobs of a character's size: ${pc(ax0)}–${pc(ax1)} % across, ${pc(ay0)}–${pc(ay1)} % down`); box=abox; snap=s2; altWon=true; N.altWon=true; pic.box=pic.boxAlt; if(r.pic) r.pic.box=pic.boxAlt; [fx0,fy0,fx1,fy1]=pic.boxAlt; }
            else logRead(id,`the AI's box passes the picture's edge — read on the 0–1000 grid it holds ${cg} blobs of a character's size, read as pixels ${cp}; the grid stays`); } /* the box's edges from the pixels (v297): an edge that cuts through the text moves out to its end, a blank margin is trimmed */
          if(snap){ const pc=v=>Math.round(v*100), mb={...box}; logRead(id,`the AI's box ${pc(fx0)}–${pc(fx1)} % across, ${pc(fy0)}–${pc(fy1)} % down, snapped to the ink: ${pc(snap.x0/W)}–${pc(snap.x1/W)} % across, ${pc(snap.y0/Hh)}–${pc(snap.y1/Hh)} % down`); box=snap;
            ...
            for(const cand of snap.beyond||[]){ const bh=cand.y1-cand.y0, room=0.3*bh, into=cand.near<0.1*bh?0.6*bh:room, sx=Math.max(0,cand.x0-room), sy=Math.max(0,cand.y0-(cand.side>0?into:room)), sw=Math.min(W,cand.x1+room)-sx, sh=Math.min(Hh,cand.y1+(cand.side>0?room:into))-sy; if(sw<4||sh<4) continue; /* a band that touches the union is a line the box cut through: the cut reaches 0.6 band heights into the union, so the reader sees the whole characters (v326) */
              ...
              if(hit){ box={x0:Math.min(box.x0,cand.x0),y0:Math.min(box.y0,cand.y0),x1:Math.max(box.x1,cand.x1),y1:Math.max(box.y1,cand.y1)}; logRead(id,`a line ${where} the AI's box, read as ${hit.t} — one of the answer's lines, the frame takes it: ${pc(box.x0/W)}–${pc(box.x1/W)} % across, ${pc(box.y0/Hh)}–${pc(box.y1/Hh)} % down`); }
              else logRead(id,`a band ${where} the AI's box read as ${got.map(l=>l.t).join(" | ")||"nothing"} — not a line of the answer, left out`); }
            /* the snap does not overrule a box the model drew around exactly its own characters (v449, H's shopfront
               江苏淮扬菜代表品牌, 2026-09-13: "eine Karte ist in Crop Mode abgeschnitten worden" — Qwen boxed nine characters
               1440 × 160 px in the copy, nine squares, and the snap kept 608 × 82: four characters with their tops sliced,
               21 % of the box's area, just past snapBox's own "specks alone" floor of a fifth per axis). A single-line box
               whose width is its height times its own character count is a measurement, not a guess — the characters ARE
               its height —, so a snap that keeps far less of that height found something else: fine print under the line,
               a bright patch, a sub-line. The model's box then stays. The two tests are coupled on purpose: a box taller
               than one line of k characters fails the first one, so the second can only fire where the height IS the
               character height. The width is deliberately NOT tested — H's 业主直租 sign (v305) legitimately loses 40 % of
               its width when the snap drops the QR code standing in the line's own band. The failure mode this accepts is
               a crop with room around the text, never one that cuts it (v380's rule). */
          ...
          if(noSplit){ N.noSplit=noSplit; logRead(id,`the AI calls these ${pic.labels.length} texts separate labels, but ${noSplit} — one card`); }
          if(pic.labels&&!noSplit){
            ...
            /* the drawing test's own inputs (v399): its width rule compares the boxes' widths in the picture's own pixels with
            ...
            if(why){ logRead(id,`the AI calls these ${lab.length} texts separate labels, but ${why} — one card`); }
              ...
              if(found&&found.hit+found.filled+found.ordered>=SPLIT_MIN){
                ...
                logRead(id,`the picture's own characters stand in ${found.bands} ${found.bands===1?"row":"rows"}, ${found.runs} runs; the reader named ${found.hit} of the ${lab.length} labels`+(found.filled?`, and ${found.filled} more by half a reading and their row's order`:"")+(found.ordered?`, and ${found.ordered} more by their row's order alone`:""));
        ...
        :`${l.zh}: no reading, its row's order gives it ${pcv(q.x0)}–${pcv(q.x1)} % across, ${pcv(q.y0)}–${pcv(q.y1)} % down`):`${l.zh}: no run of the picture reads as it — the whole picture`);
              ...
              logRead(id,plan?`the ${lab.length} labels stand in ${plan.rows} ${plan.rows===1?"row":"rows"}, and ${plan.placed} of them found their own place in the picture's own rows of characters`
          ...
          N.box=numBox(box); /* v399: the box the frame is finally built from — the snap's, or the AI's when the snap found nothing, or the union with a band the reader confirmed */
          ...
          if(picSeen.base){ const bh=box.y1-box.y0, sb=seenBase, e={top:box.y0<=0.02*Hh&&sb.y>0.005*sb.lh,bottom:box.y1>=0.98*Hh&&sb.y+sb.h<0.995*sb.lh,left:box.x0<=0.02*W&&sb.x>0.005*sb.lw,right:box.x1>=0.98*W&&sb.x+sb.w<0.995*sb.lw}; /* an edge that is the photo's own has nothing beyond it (v315: on the whole-photo proposal of H's parking sign the line said the frame reaches beyond the right edge, where nothing was) */ /* the box on the edge of the app's proposal (v313, H's 北京现代 badge: the ink rows cut the chrome characters in half at the proposal's top, Qwen boxed the visible halves at y 0–55 of 496, the snap found nothing, and the card showed half characters — "Why is the crop so wrong here?"): the text may go on beyond the edge, so the frame reaches past it — 1.5 box heights above or below, two text heights sideways — into the photo; never for the hand's frame or the whole photo, where there is nothing beyond */
            if(e.top||e.bottom||e.left||e.right){ grow={top:e.top?1.5*bh:0,bottom:e.bottom?1.5*bh:0,left:e.left?2*bh:0,right:e.right?2*bh:0}; logRead(id,`the AI's box touches the picture's ${["top","bottom","left","right"].filter(k=>e[k]).join(" and ")} edge — the frame reaches beyond it`); } } }catch(e){ box=null; logErr("snap",e&&e.message||String(e)); logRead(id,"the AI's box could not be used: "+(e&&e.message||e)); }
        /* the AI's box does not move a frame the reader placed on the very text the AI then read back (v486, H's Maison Marais
           shop sign 玛黑家, 2026-09-14: "Hier wieder total falsch gecropped" — the card's text, pinyin and meaning were right
           and its picture was a grey slab of the shopfront glass below the sign). His record says it in one line: the quick
           look read 玛黑家 at 98/97/99 % with a box per character, textLike passed and the frame went on the text — 34–70 %
           across, 38–54 % down, correct. The reading proper then ended weak (eff 149.8, the best pass read 玛墨家), so the
           picture went out, and Qwen answered THE SAME THREE CHARACTERS with a box 0.27 of the picture too low — the vertical
           drift this file has recorded on 邪不压正 and on the street sign. snapBox could not save it: the characters sit at
           267–394 of the copy and the box at 512–722, zero overlap, so its "at least half inside the AI's box" test dropped
           every blob and it returned null (N.snap null in his dump). v319's rule — the AI gets the whole proposal and its box
           may place the frame anywhere in it — then overrode a correct measurement with a guess, and cardFrame landed 56 %
           down the photo. THE BOUNDARY, and it is the only thing that makes this safe: the reader's frame survives ONLY where
           the AI agrees with the text it was placed on. v319's own case is 绿皮书, where the quick look read 绿友书一 — a
           different text — so the AI's box still wins there and everywhere else the reader was wrong about WHAT it read. Where
           the two agree, the reader has per-character boxes measured on the real pixels and the model has only confirmed the
           characters; the pixels win. Only a placement from one definite reading counts (rectOfLines, the quick look and the
           winning pass) — the close look's band is a quartile over several disagreeing passes and has no single text — and
           only while picSeen.base stands, so a v314/v318/v348/v393 re-ask, which deliberately threw the reader's frame away to
           ask again about the whole photo, is untouched. */
        const agreed=!!(placedText&&picSeen.base&&box&&placedText===cjkOnly(pic.zh));
        N.agreed=agreed?placedText.slice(0,40):null;
        if(agreed) logRead(id,`the reader placed the frame on ${placedText} and the AI read the same text back — the reader measured the characters, the AI only confirmed them: the frame stays`);
        if(box&&!agreed){ const sure=sureAngle; const cut=await frameOnText(id,picSeen.orig,seenBase,rect,seenAngle,"AI",grow,sure&&{box}); if(stale()) return; if(cut) placedCut=cut;
          ...
          if(gone) logRead(id,`a line outside the picture this card will carry, left off it: ${gone.join(" | ")}`); }
        ...
        if(splitWhole&&W&&Hh&&seenBase&&pic.labels&&pic.labels.length>=SPLIT_MIN){
          ...
          logRead(id,"the AI calls these "+pic.labels.length+" texts separate labels — one card each, all with the frame's own picture"); }
    ...
    if(weak&&pictureProvider()&&aiAutoOn()&&navigator.onLine&&picDown()){ /* v472: the provider refused a moment ago — say so, and let the no-card gate see that the picture was never judged */
      r.picErr=PIC_DOWN_TEXT(); N.picErr=String(r.picErr).slice(0,120); logRead(id,r.picErr); }
      ...
      if(!trustAngle){ logRead(id,`the straightening of ${(dk.angle||0).toFixed(1)}° is not confirmed by the reading — the AI gets the frame as it is`); }
      if(placedCut){ logRead(id,"the AI gets the whole proposal, not the placed frame's cut"); }
      picSeen=picBase;
      ...
      if(pic&&pic.dropped&&pic.dropped.length){ logRead(id,`fine print left out of the AI's answer: ${pic.dropped.join(" | ")}`); } /* Diagnostics (v312) */
    ...
    if(pic&&!pic.bad){
      ...
      SIGN[id]={lines:zh, orig:zh.slice(), conf:[], boxes:zh.map(()=>[]), img:dk.blob, angle:dk.angle||0, tightened:false, region:r, alts:guesses, trad:!!pic.zht, tradDetected:!!pic.zht, tradText:pic.zht||"",
        ai:{zh:pic.zh,zht:pic.zht,p:pic.p,m:pic.m,ml:pic.ml,desc:pic.desc||"",note:pic.note,kind:pic.kind,pageInfo:pic.pageInfo||null,ok:true,bad:false,pic:true,labels:pic.labels||null}, cardImg, weak:false};
    ...
    SIGN[id].placePic=async pic=>{ const at=READ_RUN[id]; const sg=SIGN[id];
      ...
      if(pic.dropped&&pic.dropped.length) logRead(id,`fine print left out of the AI's answer: ${pic.dropped.join(" | ")}`);
    ...
    if(AUTO[id]&&PENDING[id]&&!weak&&navigator.onLine&&SIGN[id]&&SIGN[id].aiBusy){ /* the card as read, while the check runs (v440) — only when every line is read surely (v441): a strong reading may be confident garbage, and a card that shows 人刀女 for five seconds is the wrong kind of speed. signAskAI has set aiBusy synchronously by here, so a reading the check will not wait for (no AI, no Chinese line) gets no provisional and finishPending builds at once; offline the check dies within seconds and the card follows at once anyway, and a provisional there would only run the offline model a second time */
      const low=lines.filter(l=>{ const cf=l.cf||[]; return !cf.length||cf.reduce((a,c)=>a+c,0)/cf.length<PLACE_CF; }).map(l=>`${l.t} ${l.cf&&l.cf.length?Math.round(l.cf.reduce((a,c)=>a+c,0)/l.cf.length):"–"}`);
      N.prov={held:low.length>0,low};
      if(sureLines(lines)) provisionalCard(id,SIGN[id]);
      else logRead(id,`the reading is strong, but a line is read under ${PLACE_CF} % — no card before the AI has checked it (${low.join(" | ")})`); }
...
async function saveNow(id,auto){ /* auto (v325): the card made by itself from a photo that opened by itself — no note, the shimmer, the finished card in the row */
  ...
  if(auto){ PENDING[id]=cid; AUTO[id]=true; QSCARD[id]=cid; delete QSMORE[id]; delete QSNOTE[id]; delete QSBAD[id]; } /* announced before the cut is made, so the quick look's placement meanwhile goes to PLACED and no frame is ever drawn (v325) */
...
async function provisionalCard(id,sg){
  ...
  try{
    ...
    logRead(id,"the reading is strong — the row shows the card as read while the AI checks it");
    renderShots();
...
async function splitCards(id,sg,ph){
  ...
  if(!lab||!fr||fr.length!==lab.length||lab.length<SPLIT_MIN){
    logRead(id,`the labels and their frames do not match (${lab?lab.length:0} labels, ${fr?fr.length:0} frames) — one card`); return null; }
  ...
  if(out.length<SPLIT_MIN){ logRead(id,`only ${out.length} of ${lab.length} labels could be made into cards — one card`); return null; }
...
async function finishPending(id){
  ...
  try{
    ...
    if(ph.reading.auto&&!ph.c&&sg&&SIGN[id]===sg&&sg.noText){ logRead(id,"the AI found no Chinese text in the picture — no card"); return failPending(id,"the AI found no Chinese text in the picture"); }
    ...
    const vouched=!!(sg&&sg.ai&&!sg.ai.bad&&(sg.ai.ok||sg.ai.textOk)); /* v509, H's 数码快印 of 2026-09-18: four passes read it at 95–98 %, Qwen timed out, DeepSeek kept the text and answered a meaning — and "ok" was false, since by the prompt's own rule "ok" is true only when the MEANING was already right too, which a gloss never is. The gate asked for the text's confirmation and read the meaning's; the card was refused. The text kept is the confirmation. */
    ...
    if(SPLIT[id]&&!(ph.reading.auto&&sg&&SIGN[id]===sg&&sg.ai&&sg.ai.ok&&!sg.ai.bad)) /* the last silent gate (v374): the frames were cut and the split still did not run */
      logRead(id,`the labels have their frames, but ${!ph.reading.auto?"the card was not made by the app itself":!sg||SIGN[id]!==sg?"the reading was replaced meanwhile":!sg.ai||!sg.ai.ok?"the picture answer was not used as the reading":"the AI called the picture unreadable"} — one card`);
...
async function picOnBad(sg,guesses,status,id){
  ...
  try{
    ...
    if(early){
      ...
      pic=e.pic;
      logRead(id,`the text check called the reading garbage — the picture answer asked at the quick look ${((Date.now()-early.at)/1000).toFixed(1)} s ago is used`);
    } else {
      logRead(id,"the text check called the reading garbage — the AI gets the picture");
      pic=await aiReadPicture(sg.picBlob,[...new Set(guesses.filter(Boolean))].slice(0,6),status||(()=>{}),numsFor(id)); picOk(); /* v479: this call had no record, so aiReadPicture's own lines had no photo to file under */
    }
...
async function signAskAI(id){
  ...
  sg.aiPromise=(async()=>{ try{
    ...
    if(pic){ const zh=pic.zh.split("\n"); sg.lines=zh; sg.orig=zh.slice(); sg.conf=[]; sg.boxes=zh.map(()=>[]); sg.alts=[c,...(sg.alts||[])].filter(x=>x&&x!==pic.zh).slice(0,6); sg.trad=!!pic.zht; sg.tradDetected=!!pic.zht; sg.tradText=pic.zht||""; sg.weak=false;
      sg.ai={zh:pic.zh,zht:pic.zht,p:pic.p,m:pic.m,ml:pic.ml,desc:pic.desc||"",note:pic.note,kind:pic.kind,pageInfo:pic.pageInfo||null,ok:true,bad:false,pic:true,labels:pic.labels||null}; } /* as the weak path's answer: open characters, no boxes, the reader's texts as the alternatives, the mark on the label */
    else {
    ...
    else { sg.lines=zh.split("\n"); sg.ai={zh,zht:r.zht&&CJK.test(r.zht)?recutLines(r.zht.replace(/\r/g,"").split("\n").map(l=>l.trim()).filter(Boolean).join("\n"),lines):"",p:r.p,m:r.m,ml:r.ml,desc:r.desc||"",note:r.note,kind:r.kind,ok:r.ok,bad:!!r.bad,textOk:!r.bad&&zh===c}; /* textOk (v509): the check kept the text as read — its own "ok" is false whenever it also had to replace the gloss meaning, which is nearly always, so the text's confirmation needs its own field */
...
async function readingCard(id,sg){
  ...
  if(sg.ai && c===sg.ai.zh && !sg.ai.bad && !sg.ai.kept){ mean=sg.ai.m||mean; pin=sg.ai.p||pin; mt={src:"llm",verified:true,pending:false}; if(sg.ai.m) ml=sg.ai.ml||"en"; desc=sg.ai.desc||""; descMl=sg.ai.ml||"en"; } /* the description comes with the check (v529) */
  ...
  setMl(card,ml); if(desc) setDesc(card,desc,descMl);
...
function shotDone(s){
  if(s.todo) return false; /* v509: a photo the app took in hand and that has no card yet — interrupted, failed, or taken back by hand — stays on the tab until it makes one or is deleted (H: "the image shouldn't be lost"; "bitte unter Camera belassen und als solche benennen") */
...
function clearResults(){
  for(const s of S.inbox){
    ...
    delete AUTO[s.id]; delete QSCARD[s.id]; delete QSMORE[s.id]; delete QSNOTE[s.id]; delete QSBAD[s.id];
  }
}
...
function renderInbox(main){
  ...
  main.innerHTML=`<div class="pane">
    <div class="lead">${t("Photograph a sign, a poster or a package — the card is made for you.")}</div>
...
function renderShots(){
  ...
  /* the batch, and the one thing the user has to know about it (v411, H: "Maybe we should also mention somewhere that the app
  ...
  box.querySelectorAll("[data-signcancel]").forEach(b=> b.onclick=()=>{ const id=b.dataset.signcancel; delete SIGN[id]; if(CROP&&CROP.id===id) CROP=null; S.openShot=id; renderShots(); }); /* nothing is written until saveSign, so this photo is card-less the instant SIGN goes (v468) */
...
async function importPhotos(files,opts){
  ...
  let first=null; const added=[], recs=[];
...
/* A photo the app was still working on when the app was closed is not lost (v509, H: "if I close the app while a new image is
   being processed, the image shouldn't be lost"). Measured on v508 by killing the page at each stage of a fresh photo: killed
   while the shimmer reads it, the photo came back and became its card — the placeholder card carries its `reading` and
   resumePending re-reads it (v237/v325); but killed during "Processing photo …" the file was gone entirely, since nothing
   had been written yet, and killed between the inbox write and the placeholder's the photo sat in the inbox with no card,
   no flag and no way back — hidden by v470's rule, which lists only what is being worked on, and never resumed. Two things
   close both windows. (1) The file is written to the inbox AS IT CAME, before it is downscaled (addRaw, `raw:true`), and the
   normalised copy replaces it a moment later (normalizeShot); a raw record left over at the next start is normalised then
   (normalizeRaw), through the same function. On a batch every file is written before the first is downscaled, so a close
   during the batch keeps all of them. (2) A photo that opens by itself carries `todo:true` on its record until the automatic
   card is done with it — the card written (finishPending), the reading failed (failPending, so a photo that fails is tried
   once and not for ever, v411), or the hand took it back (Cancel, Crop). At the next start every photo still marked, with no
   card from it, joins the batch queue at its head (resumeShots), so it is read exactly as a fresh photo is (autoNext →
   proposeFrame → the shimmer → the card), and the batch line says so. Only the mark decides — never the inbox at large, or
   every card-less photo on the phone would be sent to the AI (the v411 rule). What it cannot do: bring back a photo the
   system discarded while the camera app was in front — the file never reaches the page then (v316). What it costs: one
   extra IndexedDB write of the raw file per photo (a few MB, tens of milliseconds) and one small write when the photo is done. */
let _shotTs=0;
const shotTs=()=>(_shotTs=Math.max(Date.now(),_shotTs+1)); /* v509: a batch writes several photos inside one millisecond, and both the inbox's own sort and the resume read the order off ts — without this the second photo of a pick could be read first after a restart */
async function addRaw(file,shared,album){
  const ts=shotTs();
  const rec={ id:"shot_"+ts+"_"+Math.floor(Math.random()*1000), blob:file, ts, raw:true }; if(shared) rec.shared=true; /* v450: came through the share sheet (v163) — a screenshot, by the route's own design */
  if(album&&!shared) rec.album=true; /* v453: only a photo from the album is probed for a screenshot, at the normalisation */
  if(S.autoCard) rec.todo=true; /* v509: the app owes this photo a card until it is done with it */
  try{ await idbPut("inbox",rec); }catch(err){}
  return rec;
}
async function normalizeShot(rec){
  /* bake in EXIF rotation + downscale to max 1600px: keeps the inbox small
     and the OCR boxes aligned with the displayed image */
  const file=rec.blob; let blob=file, src=null;
  if(rec.album&&!rec.shared){ try{ src=await photoSource(file); }catch(err){ src=null; } } /* v453: is this a screenshot? (read off the file as it came — the downscaled copy carries no EXIF) */
  try{
    const bmp=await createImageBitmap(file);
  ...
  rec.blob=blob; delete rec.raw; delete rec.album;
  ...
  if(IMGURL[rec.id]){ URL.revokeObjectURL(IMGURL[rec.id]); delete IMGURL[rec.id]; } /* a picture drawn from the raw file is drawn again from the copy */
  try{ await idbPut("inbox",rec); }catch(err){}
}
async function normalizeRaw(){ for(const rec of S.inbox.filter(s=>s.raw)) await normalizeShot(rec); } /* v509: a file written as it came and never downscaled — the app was closed during "Processing photo …" */
function resumeShots(){ /* v509: the photos the app still owes a card, back into the batch queue at its head, in the order they were taken; a photo whose card exists only sheds its mark */
  if(!S.autoCard) return;
  const has=new Set(S.custom.filter(d=>d.shot&&d.c).map(d=>d.shot)), ids=[]; /* a card with a TEXT — the placeholder a running reading holds has none, and counting it cleared the mark of the very photo being read (found by the suite) */
  for(const s of S.inbox.filter(s=>s.todo&&!s.raw).sort((a,b)=>a.ts-b.ts)){
    if(has.has(s.id)){ todoDone(s.id); continue; }
    if(PENDING[s.id]||READING[s.id]||(CROP&&CROP.id===s.id)) continue; /* in hand right now: this run owes it nothing */
...
const RESET_KEYS=["ailog","readlog","errlog","lastRun","translateStage","tagStage","translateRun","tagRun","recheckRun",
  "resumeView","autoQueue","days","daily","lastExport","charWrites"]; /* charWrites (v512): the characters H has written, keyed by the character itself */
...
/* the screen is noted whenever the page goes away, so a reload the app did not ask for comes back to it too (v415, H: "nach unten
   swipen hat manchmal einen refresh mit zurueckspringen auf die Learn Seite zur Folge"): until v414 only reloadNow wrote resumeView,

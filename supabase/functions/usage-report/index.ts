...
// latest report row of every phone plus today's relay calls, read with the service role (see report.sql) — or, with
// {password, what:"feedback"}, the messages users sent from the app (v212, see feedback.sql).
...
Deno.serve(async (req) => {
  ...
  let payload: { password?: string; what?: string };
  ...
  const fn = payload.what === "feedback" ? "feedback_list" : "usage_latest";
  const r = await fetch(`${url}/rest/v1/rpc/${fn}`, { method: "POST", headers: { "content-type": "application/json", apikey: srv!, authorization: `Bearer ${srv}` }, body: "{}" });

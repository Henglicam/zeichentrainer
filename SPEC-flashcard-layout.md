# Flashcard layout — specification, draft 2 (2026-09-18)
...
|---|---|---|---|
| 1 Photo | the card's picture in the photo box, pinch and pan | **179** (2:1, § 4 — decided: window and box both 2:1, the deck re-cut) | never taller than 35 % of the card's room; a page/multicard front keeps `--pageh` |
...
| 5 Line | the character being written: its pinyin and meaning, one line | **40 / 0** | shown only while zone 3 is open (H: hidden when the answer is hidden); `--label2`, 15 px, ellipsis, never wraps |

Sum at 390 × 844 with one character line and the answer folded: 179 + 56 + 36 + 322 + 3 gaps of 12 = **629 of 700** — it fits without scrolling, with room for the pad to grow to the full 322. With two character lines (+64) or the answer open (+74 for the block and +52 for zone 5) it scrolls, which is what H asked for ("offen und geschlossen scrollbar"). At 360 × 740 (a friend's phone, the v474 rule) the room is about 600 px: the pad takes what is left after zones 1, 2, 3 and 5 (600 − 179 − 56 − 36 − 36 = **293**, a comfortable square — the v65 drawing sheet gives 295 on 844 and H draws on that today).
...
**The default crop format — H asked for it to be rethought, and chose the re-cut (draft 2: "Q2: re-cut").** v329 measured 21 real photos: one-line signs and labels run 2.3–6.8:1, posters, plates and two-line signs 0.9–1.6:1, almost nothing near 16:9; with a square pad under the photo a 16:9 box (201 px at 358) is 22 px over the budget of § 3. **Decision D1: the window and the box are both 2:1.** `FRAME_RATIO` becomes 2 (`windowRect`, every save path of v329 follows it by construction), `.picbox` is `aspect-ratio:2/1` on the study card, the card detail and the camera's finished card (the v223 rule: one shape on every card; the Cards tiles keep 4:3, v465, and show the wider window with the blurred fill above and below), and the shared card PNG's box follows (`cardImage`, v269). **The deck is re-cut once** through the v418 mechanism — `RECUT_V` 4, the same guards: a card with a picture, a stored `frame` and its photo is cut again at the same text frame with the 2:1 window; a card without a frame or a photo, or one whose fresh cut fails the size or correlation guard, keeps its 16:9 picture and shows it fitted in the 2:1 box with fill at the sides — the honest outcome, as at v398/v418, and `recutStat` says how many. **What it costs:** every card changes under the learner once more (a `WHATS_NEW` line), a poster's window loses about 12 % of its height against 16:9, and a split-panel label's picture (v362, the label's own frame at the photo's pixels, no window) is untouched — it never had a window and gets none. The alternatives that were offered and not taken: A, a 2:1 box over an untouched 16:9 window (no re-cut, 6 % of every picture as fill); C, a square window beside the pad (358 px of height, rejected on the arithmetic).
...
**The target.** A tap on a character button makes it the current one; the first Chinese character is current when the card lands. The current character is drawn in the pad as the **template** — its medians as strokes in `--label3` at 30 % opacity, stroke width 22 as the pad's ink, scaled into 80 % of the pad's square — and the line under the pad (zone 5) shows that character's own pinyin and meaning (`charInfo`'s data without opening a sub-row) — **only while the answer block (zone 3) is open** (draft 2, H: "Pinyin and meaning under the pad also hidden if above pinyin and meaning are hidden"): with the answer folded the pad carries the template and nothing that names it, so the card stays a test until the learner asks; the fold's one tap opens both, and the last snap of the card, which opens the block (§ 5), brings the line with it. **The next stroke to draw** is drawn over the template in the tint at 55 %, with a 10 px dot at its start (`prepStrokes`' first point). A learner who knows the character sees the whole shape and one lit stroke; one who does not sees exactly where to begin.
...
3. **Zoom and pan, the 2:1 window and the re-cut** (§ 4, D1): `verify-piczoom` — a pinch enlarging, a drag panning within the picture, the reset on the next card, the single tap still swapping crop and whole photo, the carousel not firing under a pan, the page front excluded; `verify-recut4` — a new card's window at 2:1 from `windowRect`, the detail's and the camera card's box at 2:1, a seeded 16:9 deck re-cut at boot with `recutPass` 4 and `recutStat` written, a card without a frame keeping its 16:9 picture fitted with fill, a split label's picture byte-identical, the shared PNG's box at 2:1.
...
## 13. Decided in draft 2, and what is still open

**Decided (H, 2026-09-18):** Q1 — the corner glyph is the review flag (§ 3.3). Q2 — the window and the box are 2:1 and the deck is re-cut (§ 4, D1). Q9 — the stroke order is strict, as Duolingo (§ 6). Q5 — writing is the only Learn; there is no pad-off mode and no `learnPad` setting.

**Still open — H's call, each with a recommendation:**

- **Q3 — a one-word card of two characters** (谢谢): one button each, written one after the other (recommended — the pad is per character, H's "1 character nach dem anderen"), or the word as one drawing?
...
- **Q10 — the leech flag** (H: "leech flag is dead from Learn since there is no again? What do you mean?"). Today `recordGrade` counts consecutive `again` grades in the progress row's `fails` and flags the card at `LEECH_FAILS` 4 ("failed 4 times in a row — check text, meaning and photo", v82) — the app's own signal that a card which keeps failing is usually a *bad card* rather than a bad memory. With no grade buttons nothing in Learn ever writes `again`: a card the learner cannot write is swiped past or skipped, and both write nothing, so `fails` never rises and that flag can never fire from Learn again. **Recommended: a card finished with at least one Skip counts as `again`** — `recordGrade(c,"again")` instead of `"good"`, so the card stays due, its `fails` rises, and after four skipped sessions the leech flag fires exactly as it does today. Skip is the pad's own word for "I could not", which is what `again` has always meant. The alternative — the card written with help counting as a normal review — keeps a card the learner cannot write on the same schedule as one he can, and loses the leech signal for good.

## 14. What it costs, named

A card takes longer: writing 鸡蛋 is ten to twenty seconds where *Got it* was one tap, so a session of thirty due cards is ten minutes and not one — that is the trade H is asking for, and the star's and the lock's walks are how he keeps a session short. The stroke database (`strokes.txt.gz`, 2.4 MB) is loaded at the first card of every session, not at the first drawing; it comes from the reader's cache after the first time (v141) and costs 1.2 s once. A character the database lacks falls to the print model, which is slower (a reader pass) and weaker on handwriting. **`TRACE_OK` 0.28 is unfitted**: the only real finger strokes on record are H's three 团 drawings in `DRAWLOG` (v140), and a tracing bar has to be set on his own strokes over a visible template, not on the harness's synthetic skeletons — phase 1 logs every stroke's distance in `DRAWLOG` for exactly that, and the first field session sets the number. `charWrites` grows with the characters H has written, a few kilobytes at most. And the deck's own numbers move: with no `again` written by the screen the fails counter never rises from Learn, so the leech flag of v82 is dead from this screen unless Skip counts as `again` (Q10, recommended) — the new flag rule (§ 9) covers "this card looks wrong" at creation, and Q10 is what keeps covering it after four failed attempts.

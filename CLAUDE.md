...
- **v620** — a **multicard's regions snap onto their texts** at display (`snapRegion`/`refineShot`, memory only, nothing stored), only when the ink search is sure of 60 % and the result stays on the frame — and never on a frame over half the photo or shared with another text (v628: those are the split's fallback for a text it could not place, and the search settled on any line). Why texts fall back: the model's label boxes are judged a drawing and the reader names only some labels; both cheap fixes were measured on H's photos and dropped (v628's record). A measuring grid on the AI's picture (v629) was tried and removed at v630: no better boxes, and every answer then read as a drawing, so every photo took the slow label search. **v631**: the owner's switch under Zoom check sends pictures to a faster Qwen model (`FAST_PIC` qwen3.7-flash since v632 — qwen3-vl-flash was 404 on the Token Plan; name editable) — H's speed test; a refused model falls back to the normal one for the same photo. **Owner tools → Zoom check** draws the newest 24 cards and 8 multicards; its **Data** button shares one PDF (`zdPdf`: JPEG sheets plus a `/Type/ShiziZoomData` stream), which a session unpacks back into the pictures and their frames (v627: a multicard also carries its full-size photo, its reading record and the AI's raw replies, for replaying the split) — v621–v625's share limits (300 000-byte chat cut, no zip, Chrome's size cap, the lapsed tap) are in the archive.

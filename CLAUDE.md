...
- **v595** — one cut shape for every photo: `CARD_RATIO` is **1**, so `windowRect`, `windowCut`
  ...
  waits for H's word. `spotRatios()` keeps 1.5 so an older card's photo mark still resolves.
...
{ id, c:"坚果", p:"jiānguǒ", m:"nuts", t:"Custom"|"Sign", at, v:<build that made it>,
  ...
  trad:"養樂多", ml:"de", ms:{en,de}, ds:{en,de},       // script, meaning language, meanings, descriptions
...
sentence keyed by its version; More → About lists the last five and a line slides up after an
update. **Most versions get no note — that is correct, not an oversight.** A note describing a

...
- **Tabs:** Learn · Cards · Camera · More. A tab tap always leaves the edit form; tapping **Cards** while a card is open returns to the list (v43).
...
- **Learn:** SM-2 light; progress rows are `{c, interval, ease, due, reps, fails}` — `fails` counts consecutive "again" (hard keeps it, good/easy reset it) and at `LEECH_FAILS` = 4 the card is **flagged automatically** ("failed N times in a row — check text, meaning and photo"): a leech is usually a bad card, not a bad memory. Front = photo (crop) above the text box: square 田字格 reticle up to 3 characters, wide box beyond, the font fitted so the text keeps the photo's lines and never wraps on its own (see Design). Line breaks are kept through everything: the AI answer is re-cut to the photo's lines when the model drops the `\n` (`recutLines`, same character count), a rename keeps the old break offsets (`segWithBreaks`), and the Edit form shows word cards as a textarea with **one line per photo line** (v43). Cards without line data (saved before v39) get their lines **estimated once at boot** from the crop's shape (`autoBreaks`: L ≈ √(n·h/w), cut at word boundaries, marked `lb:"auto"`); cards saved with real line data are `lb:"photo"` and never touched (v44, H: "the line break should happen automatically"). No tag row, no Reveal button — **tapping the photo or the character toggles the back on and off**. Back = pinyin with a **speaker button** (the phone's own zh-CN voice via `speechSynthesis`, nothing downloaded, shown only when such a voice exists — `ttsVoice()`, voices arrive asynchronously on Android), meaning, (context word), gloss table + context photo for sign cards, then **Again / Hard / Good / Easy** with interval preview, and underneath **⚑ Flag for review** and **✎ Edit** (`S.editFrom="study"`, Back/Save return to the revealed card). No example sentences (H's decision, v41). Empty deck → "No cards yet" + Take a photo.

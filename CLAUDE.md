...
- **v654** — **Edit on the Learn card**, grey after Star and Flag inside Whole card (H: "not too prominent"); back to the same card, the pad kept unless the characters changed.
...
- **"Whole card"** folds open at the card's foot — the characters, pinyin, meaning **and the
  description** (v585), fetched by itself 1.2 s after the card appears (v586, `explainSoon`); then Star, Flag and a grey
  **Edit** (v654) that opens the Edit form and comes back to the same card.

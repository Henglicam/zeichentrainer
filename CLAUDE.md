...
- **v654–v655** — **Edit on the Learn card** inside Whole card (H: "not too prominent"); back to the same card, the pad kept unless the characters changed. Since v655 a grey toolbar of drawn icons over labels (H chose B of three mock-ups), since v658 only Star · Edit — the photo's corner flag is the one flag (H: "Doppelt gemoppelt ist zu vermeiden"); a clean card is flagged through Edit.
...
- **"Whole card"** folds open at the card's foot — the characters, pinyin, meaning **and the
  description** (v585), fetched by itself 1.2 s after the card appears (v586, `explainSoon`); then a grey toolbar of Star
  and **Edit** (v654/v655; Flag left it at v658 — the photo's corner flag is the one) — Edit opens the Edit form and comes back to the same card.

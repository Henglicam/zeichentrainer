...
- **v620** — a **multicard's regions snap onto their texts** at display (`snapRegion`/`refineShot`, memory only, nothing stored), only when the ink search is sure of 60 % and the result stays on the frame. **Owner tools → Zoom check** draws the newest 24 cards and 8 multicards; its **Data** button shares one PDF (`zdPdf`: JPEG sheets plus a `/Type/ShiziZoomData` stream), which a session unpacks back into the pictures and their frames (v627: a multicard also carries its full-size photo, its reading record and the AI's raw replies, for replaying the split) — v621–v625's share limits (300 000-byte chat cut, no zip, Chrome's size cap, the lapsed tap) are in the archive.

...
No tab switching for the AI (v31/v32) · button always visible, never replaced by the auto result (v32) · one action after framing (v39) · plain words instead of jargon and `·` shorthand (v25/v39) · three top-bar pills overflow at 390 px, so Deck only shows off the Learn tab (v25) · Chrome/Android does not repaint after the camera until touched (v34) · Android shares `.txt` but not `.json` (v3) · a cached 404 poisoned the dictionary once (SW caches only OK responses).

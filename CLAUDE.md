...
- **v654–v655** — **Edit on the Learn card** inside Whole card (H: "not too prominent"); back to the same card, the pad kept unless the characters changed. Since v655 a grey toolbar of drawn icons over labels (H chose B of three mock-ups), since v658 only **Edit** — the marks sit on the photo like a Cards tile's: the star in the top-right corner, the flag beside it, both always drawn (H: "Doppelt gemoppelt ist zu vermeiden", "Unlogisch", then chose "marks on the photo"; v659: the flag had vanished after unflagging).
...
- **"Whole card"** folds open at the card's foot — the characters, pinyin, meaning **and the
  description** (v585), fetched by itself 1.2 s after the card appears (v586, `explainSoon`); then a grey toolbar with
  **Edit** (v654/v655; since v658 the star and the flag sit on the photo's top-right corner instead) — Edit opens the Edit form and comes back to the same card.

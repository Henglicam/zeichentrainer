...
- **v656** — a Crop again saved early lost its reading to a race: the Learn card's description, fetched for the old text, put a copy of the card into the deck while `finishPending` filled the original. `finishPending` now puts its card back; `explainCard` drops an answer for a changed text.
- **v654–v655** — **Edit on the Learn card** inside Whole card (H: "not too prominent"); back to the same card, the pad kept unless the characters changed. Since v655 Star · Flag · Edit are one grey toolbar of drawn icons over labels (H chose B of three mock-ups); starred or flagged turns its own button the tint.
...
- **"Whole card"** folds open at the card's foot — the characters, pinyin, meaning **and the
  description** (v585), fetched by itself 1.2 s after the card appears (v586, `explainSoon`); then a grey toolbar of Star, Flag
  and **Edit** (v654/v655) — Edit opens the Edit form and comes back to the same card.

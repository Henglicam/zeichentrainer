...
Open: Sign Cards M3–M4 per `SPEC-sign-cards.md` (M3: ship the Mozilla zh→en model via GitHub Action + translator worker, opt-in online translation with pending queue; M4: field polish). Idea from H (2026-09-02): the review flag queue could also be checked by an online AI (opt-in, text only, never photos or confidential text) — fits the M3 online path. Later candidates: stroke-order animations, HSK tagging.
